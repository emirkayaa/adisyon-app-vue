...
class DateCarousel extends LitElement {
  ...
  _calculateHeaderText() {
    ...
    if (firstDayOfWeekYear !== lastDayOfWeekYear) {
      ...
      headerText = `${firstDayOfWeek.toFormat("dd MM yyyy")} - ${lastDayOfWeek.toFormat("dd LLL yyyy")}`;
    } else {
      // the week is in the same year --- only show the year at the end
      headerText = `${firstDayOfWeek.toFormat("dd/MM/yyyy")} - ${lastDayOfWeek.toFormat("dd/MM/yyyy")}`;
    }

    headerText;
    return null; //headerText
  }
  ...
  _calculateDays() {
    ...
    while (currentDayCount <= 7) {
      let hafta = currentDay.toFormat("ccc");
      if (hafta == "Mon") hafta = "Pzt";
      if (hafta == "Tue") hafta = "Sal";
      if (hafta == "Wed") hafta = "Çrş";
      if (hafta == "Thu") hafta = "Prş";
      if (hafta == "Fri") hafta = "Cum";
      if (hafta == "Sat") hafta = "Cmt";
      if (hafta == "Sun") hafta = "Paz";
      days.push({
        dayOfWeek: hafta,
