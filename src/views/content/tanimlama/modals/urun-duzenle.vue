<template>
  <el-dialog v-model="dialogVisible" title="Ürün Düzenle" draggable width="90%">
    <div class="row">
...
export default {
  ...
  methods: {
    ...
    getCreateData() {
      this.$services.edit("product", this.edit_data.id).then((res) => {
        this.create_columns = res.data.data.column_set.column_arrays[0].columns;
        this.params = res.data.data.record;
        this.params.portion_json = JSON.parse(this.params.portion_json);
        this.params.product_category_id = this.params.product_category_id?.[0]?.source;
        this.params.kitchen_group_id = this.params.kitchen_group_id?.[0]?.source;
        this.params.companies_id = null;
        this.params.barcode_type_id = null;
      });
    },
    saveProduct() {
      ...
      formData.append("column_set_id", this.$store.getters.getUserData.auths.tables["product"]?.edits[0]);
      for (const clm of Object.values(this.create_columns)) {
        ...
        } else if (clm.gui_type_name == "boolean:fastchange" || clm.gui_type_name == "boolean") {
          formData.append(clm.name, this.params[clm.name] ? 1 : 0);
        } else {
          formData.append(clm.name, this.params[clm.name] == null ? "" : this.params[clm.name]);
        }
      }
      this.$services.update("product", this.edit_data.id, formData).then((res) => {
    ...
    deleteEvent() {
      this.$services.deleted("product", this.edit_data.id).then((res) => {
        if (res.data.data.message == "success") {
          ElNotification({
            title: "Başarılı",
